...
@live_only()
class ContainerAppSourceToCloudTest(ScenarioTest):
    @ResourceGroupPreparer(location="eastus2")
    def test_containerapp_up_source_to_cloud_e2e(self, resource_group):
        self.cmd('configure --defaults location={}'.format(TEST_LOCATION))

        env_name = self.create_random_name(prefix='env', length=24)
        self.cmd(f'containerapp env create -g {resource_group} -n {env_name}')
        app_name = self.create_random_name(prefix='containerapp', length=24)
        app_path = os.path.join(TEST_DIR, "NetCore7WebApp")
        self.cmd(f"containerapp up --source {app_path} --environment {env_name} -g {resource_group} -n {app_name} --ingress external --target-port 8080")
